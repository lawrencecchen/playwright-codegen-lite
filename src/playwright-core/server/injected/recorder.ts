...
import type { UIState } from "../../recorder-types";
...
class Recorder {
	...
	private async _pollRecorderMode() {
		...
		const { mode, actionPoint, actionSelector, language, testIdAttributeName } =
			state;
		...
		if (
			...
			// else this._highlight.hideActionPoint();
		...
		if (actionSelector !== this._actionSelector) {
			this._hoveredModel = actionSelector
				? querySelector(this._injectedScript, actionSelector, document)
				: null;
			this._updateHighlight();
	...
	private _shouldIgnoreMouseEvent(event: MouseEvent): boolean {
		...
		if (
			nodeName === "INPUT" &&
			["date"].includes((target as HTMLInputElement).type)
		)
			return true;
		return false;
	}
	...
	private _onMouseLeave(event: MouseEvent) {
		...
		if (
			window.top !== window &&
			this._deepEventTarget(event).nodeType === Node.DOCUMENT_NODE
		) {
	...
	private _onFocus(userGesture: boolean) {
		...
		if (userGesture) this._hoveredElement = activeElement as HTMLElement | null;
		this._updateModelForHoveredElement();
	}
