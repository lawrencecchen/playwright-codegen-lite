...
export class InjectedScript {
		...
		return {
			queryAll: (root: SelectorRoot, selector: string): Element[] => {
				...
				return allElements.filter((element) => {
					let labels: Element[] | NodeListOf<Element> | null | undefined =
					...
					if (labels === null) labels = (element as HTMLInputElement).labels;
					return (
						!!labels &&
						[...labels].some((label) =>
							matcher(elementText(this._evaluator._cacheText, label))
						)
					);
				});
			},
	...
	pollLogScale<T>(predicate: Predicate<T>): InjectedScriptPoll<T> {
		...
		let attempts = 0;
		return this.poll(predicate, (next) =>
			setTimeout(next, pollIntervals[attempts++] || 1000)
		);
	}
	...
	strictModeViolationError(
		...
		return this.createStacklessError(
			`strict mode violation: ${asLocator(
				this._sdkLanguage,
				stringifySelector(selector)
			)} resolved to ${matches.length} elements:${lines.join("")}\n`
		);
	}
	...
	private _setupHitTargetInterceptors() {
		...
		const addHitTargetInterceptorListeners = () => {
			for (const event of kAllHitTargetInterceptorEvents)
				window.addEventListener(event as any, listener, {
					capture: true,
					passive: false,
				});
		};
...
function createTextMatcher(
		...
		if (internal)
			return {
				...
				matcher: (elementText: ElementText) =>
					normalizeWhiteSpace(elementText.full) === selector,
			};
	...
	return {
		kind: "lax",
		matcher: (elementText: ElementText) =>
			normalizeWhiteSpace(elementText.full).toLowerCase().includes(selector),
	};
}
