...
function innerAsLocator(
	...
	for (let index = 0; index < parts.length - 1; index++) {
		if (
			parts[index].name === "nth" &&
			parts[index + 1].name === "internal:control" &&
			(parts[index + 1].body as string) === "enter-frame"
		) {
	...
	for (let index = 0; index < parts.length; index++) {
		...
		if (
			nextPart &&
			nextPart.name === "internal:control" &&
			(nextPart.body as string) === "enter-frame"
		) {
			locatorType = "frame";
		...
		tokens.push(
			factory.generateLocator(base, locatorType, stringifySelector(p))
		);
	}
	return tokens.join(".");
}
...
export class PythonLocatorFactory implements LocatorFactory {
	...
	private regexToString(body: RegExp) {
		...
		return `re.compile(r"${body.source
			.replace(/\\\//, "/")
			.replace(/"/g, '\\"')}"${suffix})`;
	}
...
export class JavaLocatorFactory implements LocatorFactory {
		...
		switch (kind) {
			...
			case "role":
				...
				const attrString = attrs.length
					...
					: "";
				return `getByRole(AriaRole.${toSnakeCase(
					body as string
				).toUpperCase()}${attrString})`;
			case "has-text":
				return `filter(new ${clazz}.LocatorOptions().setHasText(${this.toHasText(
					body
				)}))`;
			case "has":
	...
	private toCallWithExact(
		...
		if (isRegExp(body)) return `${method}(${this.regexToString(body)})`;
		if (exact)
			return `${method}(${this.quote(body)}, new ${clazz}.${toTitleCase(
				method
		...
		switch (kind) {
			...
			case "role":
				...
				const attrString = attrs.length
					? `, new() { ${attrs.join(", ")} }`
					: "";
				return `GetByRole(AriaRole.${toTitleCase(
					body as string
				)}${attrString})`;
			case "has-text":
